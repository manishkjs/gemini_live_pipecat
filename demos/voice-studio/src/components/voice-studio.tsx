...
import { DEFAULT_SETTINGS, LANGUAGE_OPTIONS, buildBackendPageUrl, type Engine, type SessionSettings } from "@/lib/voice-session";
...
export default function VoiceStudio({ sourceDownload = false }: { sourceDownload?: boolean }) {
  ...
  try {
    backendPages = {
      original: buildBackendPageUrl(settings.backendUrl, "original"),
      diagnostics: buildBackendPageUrl(settings.backendUrl, "diagnostics"),
    };
  } catch { /* Page shortcuts ask for a valid server URL before opening. */ }
  ...
  const choosePersona = (value: string) => {
    ...
    setSettings(current => ({ ...current, personaId: value as PersonaId, instructions: value === "custom" ? customInstructions.current : "" })); resetConversation();
  };
  ...
  const startBackend = async () => {
    if (!settings.backendUrl.trim()) { setSettingsOpen(true); return; }
    setSettingsOpen(false); resetConversation(); setSource("backend"); setPhase("connecting");
    const current = ++run.current;
    try {
      const live = await createLiveSession(settings, {
        ...
        onMessage: (role, text, append) => {
          ...
          if (append) setMessages(items => {
            ...
            if (last?.role === "assistant") {
              ...
              const separator = settings.engine === "cascade" && /\S$/.test(last.text) && /^[\p{L}\p{N}]/u.test(text) ? " " : "";
