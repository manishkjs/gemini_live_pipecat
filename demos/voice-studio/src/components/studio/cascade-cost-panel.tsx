...
import type { CascadeCost } from "@/lib/cascade-cost";
...
export default function CascadeCostPanel({ cost }: { cost: CascadeCost | null }) {
  return <details className="cascade-cost-panel">
    <summary>
      ...
      <span className="cascade-cost-stages">
        {(["stt", "llm", "tts"] as const).map(stage => {
          ...
          return <span key={stage}>{stage.toUpperCase()} {row?.disabled ? "off" : row?.complete
            ? formatCost(Number(row.known_usd)) : "pending / unavailable"}</span>;
        })}
      </span>
    </summary>
    ...
    {cost?.stages.map(stage => <div className="cascade-cost-stage" key={stage.stage}>
      ...
      <span>{stage.disabled ? "Disabled for this call" : `${stage.requests} provider requests`}</span>
