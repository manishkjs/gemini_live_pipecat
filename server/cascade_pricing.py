...
def rate_card(stage, model, provider, region="global", at=None):
    ...
    if stage == "llm" and provider == "vertex":
        prices = {
        ...
        if model not in prices:
            return None
        values = prices[model]
        if model.startswith("gemini-3") and region != "global":
...
def quote(record):
    ...
    if total is None or total != p + output + thinking:
    ...
    same_rate = card["text_in"] == card["audio_in"]
    ...
    value += Decimal(output + thinking) * Decimal(card["text_out"])
