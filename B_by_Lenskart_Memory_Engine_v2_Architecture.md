...
## 5. 8 FDE Production Lessons & Architectural Verification
...
8. **Production Was Never on AlloyDB (`Ephemeral Qdrant, Silent Data Loss`):**  
   Even with the write crash fixed, memory would still have vanished between sessions. Cloud Run logged `initialized embedded Mem0 engine with provider 'qdrant'` on every revision. `get_mem0_config()` selects `pgvector` only when `ALLOYDB_PG_DSN` / `CLOUDSQL_PG_DSN` / `DATABASE_URL` is set, else it silently picks container-local Qdrant. The DSN lived only in `server/.env`, which `.dockerignore` correctly excludes from the image, and **no matching env var was ever set on the Cloud Run service** — so the fallback fired on every cold start and each instance recycle wiped every memory. It worked on the developer laptop precisely because `.env` is present there. Fix: DSN stored in Secret Manager (`alloydb-pg-dsn`) and mounted via `--set-secrets=ALLOYDB_PG_DSN=alloydb-pg-dsn:latest`. Also raised the reachability probe from a `0.5s` socket timeout to a configurable `5s` (`PGVECTOR_PROBE_TIMEOUT`), since a cold VPC-connector dial to AlloyDB routinely exceeds half a second, and downgraded-durability now logs at `ERROR` with an explicit "memories are EPHEMERAL" warning. **Lesson: a fallback that silently trades durability for availability is worse than a hard failure — make degradation loud, and never let config that only exists in a local `.env` decide the production storage backend.**
